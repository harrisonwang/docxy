# Wharf Техническая архитектура и принципы

Этот документ подробно описывает фон, технические принципы, системную архитектуру и процесс реализации проекта Wharf.
...
Wharf — это такая служба прокси-сервера образов, целью которой является обход сетевых блокировок и ускорение загрузки образов путем самостоятельного размещения прокси-сервера.
...
Wharf реализует полный прокси для Docker Registry API, для использования требуется только добавление конфигурации прокси в клиент Docker.
